<aside id="cart-sidebar" class="fixed top-0 right-0 h-full w-full sm:w-[420px] bg-cream z-50 translate-x-full flex flex-col shadow-2xl" role="dialog" aria-modal="true" aria-label="Keranjang Belanja">
    <div class="flex items-center justify-between px-6 py-5 border-b border-sage-200">
        ...
        <button id="close-cart" class="text-sage-600 hover:text-sage-900 transition-colors p-2 rounded-full hover:bg-sage-100" aria-label="Tutup keranjang">
    ...
    <div id="cart-items" class="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        <div id="cart-empty" class="flex flex-col items-center justify-center h-64 text-center">
            ...
            <a href="{{ route('home') }}#produk" onclick="closeCart()" class="mt-4 btn-outline-sage px-6 py-2.5 text-xs inline-block">JELAJAHI PRODUK</a>
        </div>
    </div>
    <div id="cart-footer" class="hidden px-6 py-5 border-t border-sage-200 space-y-4 bg-parchment/50">
        ...
        <p class="font-sans text-xs text-sage-400">Ongkos kirim dihitung saat checkout. Gratis untuk pembelian di atas Rp 5.000.000.</p>
        <button class="btn-primary w-full py-3.5 text-xs font-sans font-semibold rounded-none">LANJUTKAN KE PEMBAYARAN</button>
